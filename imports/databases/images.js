...
if(Meteor.isServer && Meteor.isProduction){
    // We are in the server on production, we can use dropbox
    ...
    var dropbox = new Dropbox({
        accessToken: process.env.DROPBOX_ACCESS_TOKEN,  // Catching access token
        fetch: fetch
    });
}
...
export var Images = new FilesCollection({
    ...
    onAfterUpload: function(fileRef){
        if(Meteor.isProduction){
            // We are under production, so after the file upload we will send the file to Dropbox
            try{
                var self = this;
    ...
    interceptDownload: function(http, fileRef, version){
        if(Meteor.isProduction){
            // We are under production, so the files are stored in Dropbox, intercepting the download to serve the file from Dropbox
...
if (Meteor.isServer && Meteor.isProduction){
    // We are under production so files are in Dropbox, intercept File's collection remove method to remove file from DropBox
